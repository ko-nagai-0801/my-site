  ...
  return (
    <ul className="space-y-4">
      {works.map((w) => (
        <li key={w.slug}>
          <TiltCard>
            <SpotlightCard className="rounded-2xl border border-border bg-panel p-5 transition hover:border-foreground/15 sm:p-6">
              {w.meta.image ? (
              ...
              {w.meta.href || w.meta.repo || w.meta.note ? (
                <div className="mt-5 space-y-2">
                  ...
                  {w.meta.note ? <p className="text-xs leading-relaxed text-muted">{w.meta.note}</p> : null}
                </div>
              ) : null}
            </SpotlightCard>
          </TiltCard>
